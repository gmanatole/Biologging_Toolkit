...
def plot_spectrogram(inst, debut, fin, freq_min, freq_max, raw_path, nperseg = 2048, noverlap = 512, save = False, **kwargs):
	...
	ax.set_xticks(np.arange(t[0], t[-1], 25))
	ax.set_xticklabels([f"{tick:.1f}" for tick in np.arange(0, fin-debut, 25)])
	ax.set_yticks(np.arange(freq_min, freq_max, 10))
	ax.set_yticklabels([f"{tick:.0f}" for tick in np.arange(freq_min, freq_max, 10)])
