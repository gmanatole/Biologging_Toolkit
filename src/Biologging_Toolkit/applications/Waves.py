...
class Waves(Dives) :
	...
	def get_wave_period(self, samplerate = 200) :
		...
		params = butter(N=4, fs = samplerate, Wn=[1/15, 1/3], btype='bandpass', analog=False, output = 'sos')
		...
		for i in np.unique(surface_periods) :
			...
			sig, fs = sf.read(swv_fns[fn_idx], 
					 start = int((surface_time[2] - xml_start_time[fn_idx] - 1)*fs),
					 stop = int((surface_time[-3] - xml_start_time[fn_idx] + 1)*fs))
			...
			_zc_period = np.nanmedian(np.diff(zero_crossings))/samplerate*2
			...
			try :
				...
				period.append(1 / freqs[freqs > 0][psd_peaks[peak]])
			except ValueError:
