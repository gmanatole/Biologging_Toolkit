...
class Inertial():
	...
	def __init__(self, ml, T = False, S = False):
		...
		if ml == 'ml21_305a':
			self.A = self.A[:, :self.M.shape[1]]
			self.time = self.time[:self.M.shape[1]]
			self.depth = self.depth[:self.M.shape[1]]
	...
	def declinaison(self) :
		#DECLINAISON FROM NOAA already downloaded for the SES 
